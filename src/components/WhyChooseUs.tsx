...
function WhyChooseUs() {
  // Define a static pattern that will be the same on server and client
  const staticPattern = [
    [7, 1],
    [8, 2],
    [9, 3],
  ...
  return (
    <div className="py-20 lg:py-40">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10 md:gap-2 max-w-7xl mx-auto">
        {musicSchoolContent.map((content) => (
          <div
            key={content.title}
            className="relative bg-gradient-to-b dark:from-neutral-900 from-neutral-100 dark:to-neutral-950 to-white p-6 rounded-3xl overflow-hidden"
          >
            <Grid size={20} pattern={staticPattern} />
...
export const Grid = ({
  ...
  // Use the provided pattern or fall back to a default static pattern
      ...
      {squares && (
        <svg x={x} y={y} className="overflow-visible">
          {squares.map(([squareX, squareY]: any, index: number) => (
            <rect
              strokeWidth="0"
              key={`${squareX}-${squareY}-${index}`} // Use a stable key
              width={width + 1}
              height={height + 1}
              x={squareX * width}
              y={squareY * height}
